...
object DateHelper {
  ...
  /** DateTimeFormatter that emulates the very flexible joda ISODateTimeFormat.dateTimeParser. Should only be used for
    * parsing Strings into TemporalAccessor (Instant/OffsetDateTime/ZonedDateTime) and not for formatting
    * TemporalAccessor to String.
    */
  val ISODateTimeParser = new DateTimeFormatterBuilder().parseCaseInsensitive.optionalStart
  ...
  /** Returns either '2016' or '2016 - 2018' intended to be used for displaying things like the Flow copyright years
    * dynamically.
    */
  ...
  private[this] val filenameDateTimeFormatter: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyyMMdd.HHmmss.SSS")

  private[this] val TimeFormat: DateTimeFormatter = DateTimeFormatter.ofPattern("HH:mm:ss z")
  ...
  /** Returns the specified date (defaults to now) as a string like "201509"
    */
