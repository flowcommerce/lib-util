...
/** Generates a new unique ID for a resource. Main feature is to prefix the unique id with a three character identifer
  * to understand how the ID is used (e.g. 'usr')
  *
  * @param prefix
  *   Global prefix to identify the type of resource for which you are generating an ID. Must be 3 characters,
  *   lowercase.
  */
case class IdGenerator(prefix: String) {
  ...
  /** Generates an uuid based on the specified bytes. This is a pure function: the same input produces the same output.
    *
    * {{{
